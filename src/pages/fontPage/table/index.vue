<template>
  <div class="q-pa-md">
    ...
    <q-list bordered class="q-mt-lg">
      <q-expansion-item expand-separator switch-toggle-side icon="list" :label="index"
        :header-style="{ fontSize: 'large' }" v-for="(col, index) in settingColumns" class="rounded-borders">
        <q-card>
          <q-card-section>
            <div class="row">
              <div>
                <q-btn class="col q-mr-md" label="新增列" color="secondary" @click="building()" />
                <q-btn class="col q-mr-md" label="删除列" color="red" @click="building()" />
          ...
          <q-card-section class="q-pt-none">
            <div class="row justify-between">
              <div class="col q-pr-sm " v-for="item in items">
                ...
                <q-select filled v-if="item.valueType === 'aligns'" :options="['left', 'right', 'center']" class="q-mt-md"
                  dense v-for="c in col" v-model="c[item.value]" />
                <q-select filled v-if="item.valueType === 'types'"
                  :options="['primary-key', 'input', 'number', 'bool', 'other']" class="q-mt-md" dense v-for="c in col"
                  v-model="c[item.value]" />
                <q-select filled v-if="item.valueType === 'bool'" :options="[true, false]" class="q-mt-md" dense
                  v-for="c in col" v-model="c[item.value]" />
                <div v-for="c in col">
                  <q-select filled v-if="item.valueType === 'option' && c.option != null" :options="c.option"
                    class="q-mt-md" dense v-model="c.option[0]" @click="Console(c.option[0])" />
                  <q-select filled v-if="item.valueType === 'option' && c.option == null" :options="c.option"
                    class="q-mt-md" dense v-model="c.option" @click="Console(c.option[0])" />
                </div>
